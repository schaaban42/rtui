...
#    Updated: 2018/10/11 17:12:57 by schaaban         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
$(NAME): dir_creation $(OBJS)
	@echo "$(COLOR_BLUE)RTUI - $(COLOR_GREEN)Compiling...$(COLOR_WHITE)"
	@$(MAKE) -C $(LFT_DIR)
	@$(MAKE) -C $(SDL_DIR)
	@$(CC) $(FLAGS) $(OBJS) $(CC_INC) $(CC_LINK) -o $(NAME)
	@echo "$(COLOR_BLUE)RTUI - $(COLOR_GREEN)Done !$(COLOR_WHITE)"

$(OBJS_DIR)/%.o: $(SRCS_DIR)/%.c $(INC_DIR)/$(NAME).h
	@$(CC) $(FLAGS) $(CC_INC) -o $@ -c $<

dir_creation:
	@echo "$(COLOR_BLUE)RTUI - $(COLOR_GREEN)Objs creation...$(COLOR_WHITE)"
	@mkdir -p $(OBJS_DIR)

clean:
	@echo "$(COLOR_BLUE)RTUI - $(COLOR_RED)Cleaning objs...$(COLOR_WHITE)"
	@rm -rf $(OBJS_DIR)
	@$(MAKE) -C $(LFT_DIR) clean
	@$(MAKE) -C $(SDL_DIR) clean

fclean: clean
	@echo "$(COLOR_BLUE)RTUI - $(COLOR_RED)Cleaning libft...$(COLOR_WHITE)"
	@rm -rf $(NAME)
